...
export default function AnalyticsPage() {
  ...
  const scatterData = heatmap.map(p => ({ x: p.lr, y: p.cor, name: p.company_name, flag: p.discrepancy_flagged }));

  // Non-TPA portfolio only for LR charts
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <ScatterChart margin={{ top: 5, right: 20, bottom: 25, left: 5 }}>
              ...
              <XAxis dataKey="x" type="number" name="LR %" unit="%" tick={{ fontSize: 11, fill:"#94a3b8" }}
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={portfolio} barSize={44}>
              ...
              <Bar dataKey="total_premium" name="Total Premium" radius={[6,6,0,0]}>
                {portfolio.map((r, i) => (
                  <Cell key={i} fill={r.segment === "TPA" ? "#14b8a6" : r.segment === "CORPORATE" ? "#002F6C" : "#8b5cf6"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={timeline}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="count" stroke="#002F6C" strokeWidth={2.5} dot={{ r:4, fill:"#002F6C" }} name="Schemes" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="card">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[750px]">
            ...
            <tbody>
              {heatmap.map((p, i) => {
                const risk = p.cor >= 115 ? { l:"CRITICAL", c:"bg-red-100 text-red-700" }
                ...
                return (
                  <tr key={p.policy_number} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="table-td"><span className={cn("badge", risk.c)}>{risk.l}</span></td>
