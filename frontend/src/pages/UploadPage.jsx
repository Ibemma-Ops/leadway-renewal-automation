...
function BatchResultPanel({ batch, onDelete, canDelete, deleting }) {
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-4 text-xs">
            <div className="text-center">
              ...
              <p className="font-bold text-gray-700 text-lg">{batch.total_records}</p>
            </div>
            <div className="text-center">
              <p className="text-green-600">Processed</p>
              <p className="font-bold text-green-700 text-lg">{batch.processed_records}</p>
            </div>
            <div className="text-center">
              <p className="text-teal-600 flex items-center gap-1">
                <Building2 size={10} /> TPA Routed
              </p>
            ...
            <div className="text-center">
              ...
              <p className="font-bold text-red-600 text-lg">{batch.failed_records}</p>
            </div>
          </div>
      ...
      {(batch.tpa_routed || 0) > 0 && (
        <div className="mt-3 flex items-center gap-2 bg-teal-50 border border-teal-200 rounded-lg px-3 py-2">
          ...
          <p className="text-xs text-teal-700">
            {batch.tpa_routed} TPA scheme{batch.tpa_routed > 1 ? "s" : ""} routed to TPA desk —
            excluded from automated renewal pipeline.
          </p>
        </div>
      )}
      ...
      {(batch.flagged_records || 0) > 0 && (
        <div className="mt-2 flex items-center gap-2 bg-rose-50 border border-rose-200 rounded-lg px-3 py-2">
          ...
          <p className="text-xs text-rose-700">
            {batch.flagged_records} scheme{batch.flagged_records > 1 ? "s have" : " has"} LR/COR discrepancy
            &gt;1% vs workbook values — routed to Underwriter acknowledgement queue.
          </p>
        </div>
      )}
          ...
          {expanded && (
            <div className="mt-2 space-y-1 max-h-48 overflow-y-auto">
              {batch.error_details.errors?.map((e, i) => (
                <p
                  key={i}
                  className="text-xs text-red-600 bg-red-50/60 rounded px-2 py-1 font-mono"
                >
                  ✕ {e}
                </p>
              ))}
              {batch.error_details.warnings?.map((w, i) => (
                <p
                  key={i}
                  className="text-xs text-yellow-700 bg-yellow-50/60 rounded px-2 py-1 font-mono"
                >
                  ⚠ {w}
                </p>
              ))}
...
export default function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setFile(null);
      toast.success(
        `Ingested ${res.data.processed_records} policies · ${res.data.tpa_routed || 0} TPA routed`
      );
      loadHistory();
    } catch (e) {
  ...
  const handleDeleteBatch = async (batchId) => {
    ...
    try {
      await uploadAPI.delete(batchId);
      toast.success("Upload deleted successfully");
      if (result?.id === batchId) setResult(null);
      loadHistory();
    } catch (err) {
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        title="Excel Upload"
        subtitle="Ingest renewal data — Corporate, Retail, and TPA sheets"
      />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              {[
                ...
                <div key={sheet} className="bg-white/70 rounded-lg p-3">
                  <p className={cn("font-bold inline-block px-2 py-0.5 rounded text-xs mb-1", cls)}>
                    {sheet}
                  </p>
      ...
      <div
        ...
        className={cn(
          ...
          dragging
            ? "border-brand-blue bg-brand-blue-50 scale-[1.01]"
            : file
