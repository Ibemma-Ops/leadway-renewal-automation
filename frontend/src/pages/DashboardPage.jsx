...
import { StatCard, Loading, PageHeader } from "../components/common";
import {
  BarChart,
  Bar,
  XAxis,
...
const PIE_COLORS = ["#E30613", "#F58220", "#111111", "#10b981", "#8b5cf6", "#14b8a6", "#6b7280"];
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-100 min-h-screen">
      <PageHeader
        ...
        subtitle={`Portfolio overview · ${new Date().toLocaleDateString("en-GB", {
          weekday: "long",
          day: "numeric",
          month: "long",
          year: "numeric",
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Policies" value={stats?.total_policies ?? 0} icon={FileText} color="red" />
        <StatCard label="TPA Routed" value={stats?.tpa_routed ?? 0} icon={Building2} color="teal" sub="Excluded from auto pipeline" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Awaiting Sales" value={stats?.awaiting_sales ?? 0} icon={UserCheck} color="yellow" alert={stats?.awaiting_sales > 0} />
        <StatCard label="Awaiting Underwriter" value={stats?.awaiting_underwriter ?? 0} icon={ShieldAlert} color="orange" alert={stats?.awaiting_underwriter > 0} />
        <StatCard label="Awaiting HBD" value={stats?.awaiting_hbd ?? 0} icon={TrendingUp} color="purple" />
        <StatCard label="Awaiting MD/CEO" value={stats?.awaiting_md_ceo ?? 0} icon={TrendingUp} color="red" />
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Premium at Risk" value={formatCurrency(stats?.total_premium_at_risk)} icon={DollarSign} color="red" sub="Pending approval schemes" />
        <StatCard
          label="Avg Loss Ratio"
          value={`${stats?.avg_lr ?? 0}%`}
          icon={TrendingUp}
          ...
          {timeline.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={timeline} barSize={22}>
                ...
                <Bar dataKey="count" fill="#E30613" radius={[4, 4, 0, 0]} name="Policies" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
              ...
              <tbody>
                {portfolio.map((r) => (
                  <tr key={r.segment} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className={cn("table-td text-right font-semibold", r.segment === "TPA" ? "text-gray-400" : COR_COLOR(r.avg_lr))}>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={sectorLR} barSize={28} layout="vertical" margin={{ left: 120 }}>
              ...
              <Bar dataKey="avg_lr" name="Avg LR %" radius={[0, 4, 4, 0]}>
                {sectorLR.map((r, i) => (
                  <Cell key={i} fill={r.avg_lr >= 95 ? "#E30613" : r.avg_lr >= 80 ? "#F58220" : "#10b981"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
              ...
              return (
                <div key={p.policy_number} className={cn("border rounded-lg p-3", styles.card)}>
                  ...
                  <div className="flex items-end justify-between mb-1.5">
                    <div>
                      <p className="text-xs text-gray-500">LR</p>
                      <p className={cn("text-sm font-bold", styles.text)}>{formatPct(p.lr)}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-gray-500">COR</p>
                      <p className={cn("text-sm font-bold", styles.text)}>{formatPct(p.cor)}</p>
                    </div>
                  </div>
                  ...
                  {(p.risk_flags || []).length > 0 && (
                    <div className="mt-1 flex flex-wrap gap-0.5">
                      {p.risk_flags.filter((f) => f !== "HIGH_COR").map((f) => (
                        <span key={f} className="text-xs bg-white/70 text-gray-600 px-1 rounded">
                          {f.replace(/_/g, " ")}
                        </span>
                      ))}
                    </div>
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {Object.entries(riskFlags).map(([flag, count]) => {
              ...
              return (
                <div key={flag} className="rounded-xl bg-gray-50 border border-gray-100 p-3 text-center">
                  <p className="text-2xl font-bold text-brand-black">{count}</p>
                  <p className={cn("inline-flex mt-2 px-2 py-1 rounded-full text-xs font-medium", meta.badgeClass)}>
                    {meta.label}
                  </p>
                </div>
              );
